...
```Shell
docker pull openls/pos-homologation-ve:alpine
```
 ...
 * `DB_NAME`: Database name that pos-homologation-ve will use to connect with the database. Default: `adempiere`.
 * `DB_USER`: Database user that pos-homologation-ve will use to connect with the database. Default: `adempiere`.
 ...
 * `SERVER_PORT`: Port to access pos-homologation-ve from outside of the container. Default: `50059`.
...
```Shell
docker run -d -p 50059:50059 --name pos-homologation-ve -e DB_HOST="localhost" -e DB_PORT=5432 -e DB_NAME="adempiere" -e DB_USER="adempiere" -e DB_PASSWORD="adempiere" openls/pos-homologation-ve:alpine
```

See all images [here](https://hub.docker.com/r/openls/pos-homologation-ve)
